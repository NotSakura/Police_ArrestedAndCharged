...
Through out the later sections, there will be more specific graphs that explicitly show the connection between each sub-category and number of arrests made such as year, neighbourhood, and charges. In this small sub-section is shown a small snippet (@tbl-modelresults) of all the independent variables (year, neighborhood, charges) together with the dependent variable (arrest count). In the discussion section we will take these results and show that arrest count in fact is a dependent variable.
