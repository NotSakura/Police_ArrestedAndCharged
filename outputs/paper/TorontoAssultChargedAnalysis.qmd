...
Through out the later sections, there will be more specific graphs that explicitly show the connection between each sub-category and number of arrests made such as year, neighbourhood, charges and sex. In this small sub-section is shown a small snippet (@tbl-modelresults) of all the independent variables (year, neighborhood, charges, sex) together with the dependent variable (arrest count). In the discussion section we will take these results and show that arrest count in fact is a dependent variable.
...
toronto_arrested_clean <-
  ...
  select(arrest_year, category, subtype, neighbourhood_158, arrest_count)
...
#| echo: false
#| label: tbl-ward
#| tbl-cap: Total arrested in Toronto from 2014, 2022 divided by neighbourhood/ward
## Now add the years for a cleaner data
data_ward <- toronto_arrested_clean |>
  group_by(neighbourhood_158) |>
  summarize(arrest_count = sum(arrest_count))

write_csv(
  x = data_year,
  file = "/cloud/project/outputs/data/TorontoArrestCleanByWard.csv"
)

#head(data_year)
head(data_ward) |> kable(format = "pipe", padding = 2, col.names = c("Ward", "Arrest Count"))

```


## Catagory and Subtype Vs Arrest Count
```{r}
#| echo: false
#| label: tbl-modelresults3
#| tbl-cap: catagory and Subtype Vs Arrest Count
#| tbl-subcap:
#| - "Category "
#| - "Subtype "
#| layout-ncol: 1 
...
# add by subtype
data_sub <- toronto_arrested_clean |>
  group_by(subtype) |>
  summarize(arrest_count = sum(arrest_count))

write_csv(
  x = data_cat,
  file = "/cloud/project/outputs/data/TorontoArrestCleanbySubtype.csv"
)

data_cat |> kable(format = "pipe", padding = 2, col.names = c("Category", "Arrest Count"))
data_sub |> kable(format = "pipe", padding = 2, col.names = c("Subtype", "Arrest Count")) #too big
...
#| echo: false
#| label: arrest/year
#| fig-cap: Realting arrest count with arrest 
